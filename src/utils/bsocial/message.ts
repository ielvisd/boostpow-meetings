...
export async function sendMessage({
  ...
  const script = Script.fromASM(
    'OP_0 OP_RETURN ' +
      dataPayload.map((str) => bops.to(bops.from(str, 'utf8'), 'hex')).join(' ')
  );

  console.log(script.toASM());

  const outputs: { script: string; amount: number; currency: string }[] = [
    { script: script.toASM(), amount: 0, currency: 'BSV' },
  ];

  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  //@ts-ignore
  const resp = await window.relayone.send({
    to: 'powco@relayx.io',
    currency: 'USD',
    amount: 0.0001,
  ...
  (async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        'https://onchain.sv/api/v1/transactions',
        {
          transaction: resp.txhex,
        }
      );
      ...
      const { data: getData } = await axios.get(
        `https://onchain.sv/api/v1/transactions/${resp.txid}`
      );
